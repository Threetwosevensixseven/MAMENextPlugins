...
To install, download the `nextfaststart.zip` file from the latest release, and unzip it into your configured [MAME plugins folder](https://docs.mamedev.org/plugins/index.html#using-plugins) folder, so that the MAME plugins folder contains a `nextfaststart` folder with the `.lua` and .json` files inside it.
